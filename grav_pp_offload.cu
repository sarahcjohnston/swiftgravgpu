...
extern "C" void pp_offload(
  ...
  cudaMemcpyAsync(d_multi_j, multi_j, sizeof(multipole),
                  cudaMemcpyHostToDevice);
  multipole *d_multi_i;
  cudaMalloc(&d_multi_i, sizeof(multipole));
  cudaMemcpyAsync(d_multi_i, multi_i, sizeof(multipole),
                  cudaMemcpyHostToDevice);
  ...
  // copy data to device
  cudaMemcpyAsync(d_h_i, h_i, *gcount_i * sizeof(float),
                  cudaMemcpyHostToDevice);
  ...
  cudaMemcpyAsync(d_mass_j, mass_j_arr, *gcount_padded_j * sizeof(float),
                  cudaMemcpyHostToDevice);
  cudaMemcpyAsync(d_x_i, x_i, *gcount_i * sizeof(float),
                  cudaMemcpyHostToDevice);
  cudaMemcpyAsync(d_x_j, x_j_arr, *gcount_padded_j * sizeof(float),
                  cudaMemcpyHostToDevice);
  cudaMemcpyAsync(d_y_i, y_i, *gcount_i * sizeof(float),
                  cudaMemcpyHostToDevice);
  cudaMemcpyAsync(d_y_j, y_j_arr, *gcount_padded_j * sizeof(float),
                  cudaMemcpyHostToDevice);
  cudaMemcpyAsync(d_z_i, z_i, *gcount_i * sizeof(float),
                  cudaMemcpyHostToDevice);
